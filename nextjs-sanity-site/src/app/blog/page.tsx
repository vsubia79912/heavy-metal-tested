...
async function getPosts(): Promise<PostListItem[]> {
  try {
    ...
    console.log('📋 All posts (raw):', allPosts)
    console.log('📊 Total posts found:', allPosts?.length || 0)
    
    // Test posts with slug
    console.log('🔍 Testing posts with slug...')
    const postsWithSlug = await sanityClient.fetch(`*[_type == "post" && defined(slug)]`)
    console.log('📋 Posts with slug:', postsWithSlug)
    
    // Test posts with slug.current
    console.log('🔍 Testing posts with slug.current...')
    const postsWithSlugCurrent = await sanityClient.fetch(`*[_type == "post" && defined(slug.current)]`)
    console.log('📋 Posts with slug.current:', postsWithSlugCurrent)
    
    // Now try the original query
    console.log('🔍 Fetching posts with original query:', postsQuery)
    const posts = await sanityClient.fetch(postsQuery)
    console.log('📝 Final fetched posts:', posts)
